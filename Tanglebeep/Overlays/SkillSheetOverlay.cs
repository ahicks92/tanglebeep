...
namespace Tanglebeep.Overlays {
    ...
    /// slot on bar 1, Ctrl+1-8 to bar 2) plus an <b>other abilities</b> row for actives not tied to a
    /// real job (chiefly weapon masteries, learned from the pseudo-job GENERIC), and a <b>show
    /// unlearned</b> toggle that adds every job's
    ...
    internal sealed class SkillSheetOverlay : IUiOverlay {
        ...
        private static void BuildActiveJobRows(IOverlayBuilder builder, HeroPC hero) {
            ...
            var emitted = new HashSet<CharacterJobs>();
            foreach (CharacterJobData job in GameMasterScript.masterJobList) {
                if (!byJob.TryGetValue(job.jobEnum, out List<AbilityScript> actives) || actives.Count == 0) {
                    continue;
                }

                emitted.Add(job.jobEnum);
                BuildActiveRow(
                    builder,
                    job.jobEnum.ToString(),
                    ModStrings.JobAbilitiesRow(GameLabelReader.Clean(job.DisplayName)),
                    actives,
                    hero
                );
            }

            // Whatever's left isn't tied to a real job — chiefly weapon masteries, whose grant abilities
            // (e.g. dagger mastery's "Lightning Jab") are learned from the pseudo-job GENERIC, which is
            // never in masterJobList. Without this pass those actives get collected above and then
            // silently dropped. Fold every leftover bucket into one "other abilities" row.
            var others = new List<AbilityScript>();
            ...
            if (others.Count > 0) {
                BuildActiveRow(builder, "other", ModStrings.OtherAbilitiesRow, others, hero);
            }
        }

        private static void BuildActiveRow(
            IOverlayBuilder builder,
            string rowKey,
            string rowLabel,
            ...
            builder.StartRow("job:" + rowKey);
            builder.AddLabel(
                ControlId.Structural("skill:jobrow:" + rowKey),
                ctx => ctx.Message.Fragment(rowLabel)
            );
